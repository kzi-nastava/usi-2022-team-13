...
namespace HealthCareSystem.Core.Users.Doctors.Repository
{
    class DoctorRepository
    {
        ...
        public DoctorAnamnesis GetDoctorAnamnesis(int examinationId, ExaminationRepository examinationRepository)
        {
            if (examinationRepository.Connection.State == System.Data.ConnectionState.Closed) examinationRepository.Connection.Open();
            ...
            OleDbCommand cmd = DatabaseCommander.GetCommand(query, examinationRepository.Connection);
        ...
        public bool IsDoctorAvailableAtTime(int doctorId, DateTime startDate, List<Examination> takenExaminations, ExaminationRepository examinationRepository)
        {
            bool isAvailable = true;
            foreach (Examination takenExam in takenExaminations)
            {
                if (!examinationRepository.DoctorRep.IsValidTimeAndDoctor(startDate, takenExam, doctorId)) 
                    isAvailable = false;  
            }
            return isAvailable;
        }

        public bool IsDoctorEqual(int doctorId, Examination takenExamination)
        {
            return doctorId == takenExamination.IdDoctor;
        }

        public bool IsValidTimeAndDoctor(DateTime startDate, Examination takenExamination, int doctorId)
        {

            return !(!TimeDateHelpers.IsValidTime(startDate, takenExamination) && this.IsDoctorEqual(doctorId, takenExamination));
        }
    }
}
