...
namespace HealthCareSystem.Core.Scripts.Repository
{
    class InsertionRepository
    {
        ...
        private void ExecuteRegularRenovations(Renovation renovation)
        {
            ...
            DatabaseCommander.ExecuteNonQueries(deleteRegularRenovationQuery, _connection);
        }
        ...
        private void ExecuteMergingRenovations(Renovation renovation)
        {
            ...
            foreach (RoomHasEquipment equipment in equipmentToBeMoved)
            {
                ...
                if (checkNumber.Count == 0)
                {
                    ...
                    DatabaseCommander.ExecuteNonQueries(insertQueryDestination, _connection);
                }
                ...
                DatabaseCommander.ExecuteNonQueries(updateFirstRoomEquipment, _connection);
            ...
            DatabaseCommander.ExecuteNonQueries(deleteMergingRenovationQuery, _connection);


            string deleteOtherRoomQuery = "delete from Rooms where ID = " + renovation.SecondRoomId;
            DatabaseCommander.ExecuteNonQueries(deleteOtherRoomQuery, _connection);
        }
        ...
        private void ExecuteSplitingRenovations(Renovation renovation)
        {
            ...
            foreach (RoomHasEquipment equipment in equipmentToBeMovedToWarehouse)
            {
                ...
                DatabaseCommander.ExecuteNonQueries(updateWarehouseEquipment, _connection);

            }

            string deleteSplittingRenovationQuery = "delete from Renovations where id_room = " + renovation.RoomId;
            DatabaseCommander.ExecuteNonQueries(deleteSplittingRenovationQuery, _connection);
        ...
        {
            DatabaseCommander.ExecuteNonQueries("Delete from " + tableName, _connection);
        }

        private static List<String> GetUserIDs(UserRole role)
        {
            var query = "select ID from Users where role='" + role.ToString() + "'";
            return DatabaseCommander.ExecuteReaderQueries(query, _connection);
        }
        ...
        {
            DatabaseCommander.ExecuteNonQueries(query, _connection);
        }
        ...
        {
            List<string> patientIDs = GetPatientIds();
            List<string> ingredientIDs = DatabaseCommander.ExecuteReaderQueries("select id from ingredients", _connection);
        ...
        private static List<MedicationsIngredient> GetMedicationsIngredients()
        {
            ...
            List<String> medicationsIDs = GetMedicationIds();
            List<String> ingredientsIDs = DatabaseCommander.ExecuteReaderQueries("select id from Ingredients", _connection);
        ...
        {
            List<string> examinationIds = DatabaseCommander.ExecuteReaderQueries("select id from examination", _connection);
            List<string> doctors = GetDoctorIds();
            List<string> rooms = DatabaseCommander.ExecuteReaderQueries("select id from rooms where type = '" + TypeOfRoom.ExaminationRoom.ToString() + "'", _connection);
        ...
        private static List<Receipt> GetReceipts()
        {
            ...
            List<string> patientIds = GetPatientIds();
            List<string> doctorIds = GetDoctorIds();
            List<string> instructionIds = DatabaseCommander.ExecuteReaderQueries("select id from Instructions", _connection);
        ...
        {
            List<string> receiptIds = DatabaseCommander.ExecuteReaderQueries("select id from Receipt", _connection);
